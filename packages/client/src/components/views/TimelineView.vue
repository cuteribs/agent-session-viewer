...
function summarizeArgs(args: Record<string, unknown>, toolName?: string): string {
  ...
  const priorityKeys = toolName === 'task'
    ? taskKeys
      ...
      >
        <div data-name="message-header" class="flex items-center justify-between px-4 py-2 bg-surface-container-low rounded-t-lg">
          <div class="flex items-center gap-3 min-w-0">
            ...
            <span
              v-if="node.message.role === 'tool' && node.message.toolCalls?.[0]?.name"
              data-name="message-tool-name"
              class="text-xs font-semibold text-on-surface truncate"
            >
              {{ node.message.toolCalls[0].name }}<template v-if="node.message.toolCalls[0].name === 'task' && node.message.toolCalls[0].arguments?.agent_type"> <span class="font-normal opacity-60">({{ node.message.toolCalls[0].arguments.agent_type }})</span></template>
            </span>
        ...
        <div data-name="message-body" class="px-4">
          <p v-if="node.message.content && node.message.role !== 'tool'" data-name="message-content-text" class="text-sm text-on-surface whitespace-pre-wrap break-words py-3 font-mono">
            {{ truncateText(node.message.content, 500) }}
          </p>
          <p
            v-else-if="node.message.role === 'tool' && node.message.toolCalls?.[0]?.arguments"
            data-name="message-content-text"
            class="text-sm text-on-surface-variant py-2 font-mono truncate"
          ...
          <div
            v-if="node.message.toolResult"
            data-name="message-tool-result"
            :class="[
          ...
          >
            ...
            <p
              v-if="node.message.toolResult.content"
              class="px-2 py-1 text-xs font-mono text-on-surface-variant truncate"
              :title="node.message.toolResult.content"
            >{{ node.message.toolResult.content.length > 100 ? node.message.toolResult.content.substring(0, 100) + '…' : node.message.toolResult.content }}</p>
          </div>
        </div>
      </div>
    </div>
