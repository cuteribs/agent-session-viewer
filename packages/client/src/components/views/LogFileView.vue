...
<template>
  <div data-name="logfile-view" class="space-y-4">
    <!-- Log file location -->
    <div data-name="logfile-location" class="bg-surface rounded-lg border border-outline-variant p-4">
      <h3 class="text-sm font-medium text-on-surface mb-2">Raw log file</h3>
      ...
      <div v-if="isDbBacked || !logPath" data-name="logfile-db-note" class="space-y-3">
        ...
        <a
          ...
          target="_blank"
          rel="noopener"
          class="inline-flex items-center gap-1 px-3 py-1.5 text-sm bg-primary text-white rounded hover:bg-primary/90 transition-colors"
        >
      ...
      <div v-else class="space-y-3">
        <div data-name="logfile-path-row" class="flex items-stretch gap-2">
          <code
            ...
            class="flex-1 px-3 py-2 bg-surface-container-high rounded text-sm text-on-surface font-mono break-all select-all"
          >{{ logPath }}</code>
          <button
            data-name="logfile-copy"
            @click="copyPath"
            class="flex items-center gap-1 px-3 py-2 text-sm bg-surface-container-high hover:bg-gray-200 dark:hover:bg-gray-600 rounded transition-colors shrink-0"
          >
            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
