...
// Sessions API
export async function fetchSessions(source?: 'claude' | 'copilot' | 'codex' | 'opencode' | 'vscode' | 'all') {
  const query = source ? `?source=${source}` : '';
  return fetchJSON<import('shared/types').SessionSummary[]>(`/sessions${query}`);
}

export async function fetchSession(source: 'claude' | 'copilot' | 'codex' | 'opencode' | 'vscode', sessionId: string) {
...
export async function fetchSessionStats(source: 'claude' | 'copilot' | 'codex' | 'opencode' | 'vscode', sessionId: string) {
